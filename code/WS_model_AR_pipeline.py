...
dictionaryFile = pd.read_excel("../data/financial_dictionary.xlsx", engine='openpyxl')
dictionaryData = dictionaryFile.Word
financialWordsSet = set(dictionaryData.unique())
raw_data_path = '../data/raw_AR_Data/'
preprocessed_data_path = '../data/NewPreprocessedData/'
labelled_data_path = '../data/NewWeakLabelledData/'
...
def preprocess_Data():
    for file in glob.glob(raw_data_path + "*.txt"):
        idx = 0
        df1 = pd.DataFrame(columns=['sentence', 'numerical', 'financial'])
        ...
        for sentence in text:
            ...
            df1.loc[idx] = [sentence, check_for_numeric_data(sentence), financial_filter_data(sentence)]
            idx += 1
...
def label_Data(start, end, master_list):
    for file_manual in master_list[start:end]:
        if (not os.path.exists(labelled_data_path + file_manual.split('/')[-1])):
            try:
                df_train = pd.read_excel(file_manual, engine='openpyxl')
                ...
                df_train.to_excel(labelled_data_path + file_manual.split('/')[-1], index=False)
            except:
                print(file_manual)
...
def main():
    ...
    number_threads = 32
    master_list = glob.glob(preprocessed_data_path + "*.xlsx")

    start, end = 0, len(master_list)
    # end = 1000
    print('number of files', end)
    if end % number_threads:
        end = (int(end/number_threads) + 1) * number_threads
    threads = [ multiprocessing.Process(target = label_Data, kwargs={'start':int(i), 'end':int(j), 'master_list':master_list}) for (i,j) in [((end-start)/number_threads*k, (end-start)/number_threads*(k+1)) for k in range(number_threads)] ]
