...
def fine_tune_plm(gpu_numbers: str, train_data_path: str, language_model_to_use: str, seed: int, batch_size: int, learning_rate: float, save_model_path: str):
    """
    ...
    try: 
        if language_model_to_use == 'bert':
            ...
            model = BertForSequenceClassification.from_pretrained('../finbert-uncased/model', num_labels=2).to(device)
        elif language_model_to_use == 'flangbert':
            model = BertForSequenceClassification.from_pretrained('SALT-NLP/FLANG-BERT', num_labels=2).to(device)
        elif language_model_to_use == 'bert-large':
    ...
    # test 1
    # load test data 1
    data_df_test = pd.read_excel("../data/test/gold_arpts_numclaim-test-" + str(seed) + ".xlsx")
    ...
    # load test data 2
    data_df_test = pd.read_excel("../data/test/gold_ec_numclaim-test-" + str(seed) + ".xlsx")
    sentences_test = data_df_test['text'].to_list()
    labels_test = data_df_test['label'].to_numpy()
    ...
    for i, seed in enumerate(seeds):
        for k, batch_size in enumerate(batch_sizes):
            for j, learning_rate in enumerate(learning_rates):
                ...
                results.append(fine_tune_plm(gpu_numbers, train_data_path, language_model_to_use, seed, batch_size, learning_rate, None))
                ...
                df.to_excel(f'../data/grid_search_results/train_ec_{data_category}_{language_model_to_use}.xlsx', index=False)
...
if __name__=='__main__':
    ...
    # experiments
    for language_model_to_use in ["roberta", "roberta-large", "bert", "bert-large", "finbert", "flangbert"]:# provide list of models
        data_category = "gold_ec_numclaim"
